...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!ghlApiKey || ghlApiKey.length === 0) {
      return new Response(JSON.stringify({ 
        configured: false, 
        message: 'GoHighLevel API key not configured. Add GHL_API_KEY to secrets.' 
      }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
